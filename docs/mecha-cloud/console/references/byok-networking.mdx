...
import configureByokUrl from './assets/configure-byok.png';
...
1. **Enhanced Security and Privacy Control:**
...
5. **Minimized Risk of Vendor Lock-in:**
...
<img src={configureByokUrl} alt="Logs on Grafana 1" />;

“A configuration modal will appear to help you set up the network: 
1. **Select the Network**: Choose the appropriate network from the dropdown list. 
    * In this example, "SwishNet / swishnet" is selected. 
2. **Enter the Network Key**: 
    * Use the BYOK (Bring Your Own Key) key that you configured while adding the network. 
3. **Input the IPv4 Address**: 
...
1. **Check the Machine Listing**: Verify that the IP address is now linked to your machine as configured.
2. **Inspect Machine Details**: Go to **Settings > Networking** within the machine details page to confirm the network settings.
3. **Review Network Details**: Ensure that the machine appears in the network nodes table under the selected network.
